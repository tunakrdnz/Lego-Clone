...
let slidesContainer = document.querySelector(".featured-sets-wholebody-container");
...
let totalWidth = percentageMove*eachSlideWidth + Number(slidesContainer.style.gap.slice(0, -2))*(percentageMove-1);
...
nextBtn.addEventListener("click", function (){
    ...
    if (noOfClicks === -1){
        ...
        slides.forEach((slide) => {
            slide.style.transform = `translateX(${slidesContainer.clientWidth*noOfClicks - Number(slidesContainer.style.gap.slice(0, -2))}px)`})
    ...
    runnerBar.style.transform = `translateX(${noOfClicks*trial*runnerBar.clientWidth}px)`;
    slides.forEach((slide) => {
    slide.style.transform = `translateX(${slidesContainer.clientWidth*noOfClicks + noOfClicks*Number(slidesContainer.style.gap.slice(0, -2))}px)`})
    noOfClicks--}
...
function calculateOnResize(){
    ...
    totalWidth = percentageMove*eachSlideWidth + Number(slidesContainer.style.gap.slice(0, -2))*(percentageMove-1);
    slidesContainer.style.width = `${totalWidth}px`;
